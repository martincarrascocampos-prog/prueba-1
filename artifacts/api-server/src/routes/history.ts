...
function buildTopicResult(
  ...
  const votes = data.votesByTopic.get(t.id) ?? [];
  ...
  const r = computeTopicResult(t.status, votes, members, attendeeIds, checkedOutIds, t.weighted);
  return {
    type: t.type,
    candidateMode: t.candidateMode,
    weighted: t.weighted,
...
// admin-only distinction. Their vote (if any) is shown but never counted.
function buildTopicBallots(
  ...
  const votes = data.votesByTopic.get(t.id) ?? [];
  const votesByUser = new Map<number, VoteRow[]>();
  ...
  const labelFor = (rows: VoteRow[]): string => {
  ...
  return eligibleMembersFor(data, roster, t.id)
    .map((m) => {
...
router.get("/public/history", async (_req, res): Promise<void> => {
  ...
  const result = data.sessions.map((s) => {
    ...
    // attendees (the retired-early distinction is admin-only). Vote tallies
    // keep excluding their weight via checkedOutIds — that logic is untouched.
    const attendedIds = new Set([...attendeeIds, ...checkedOutIds]);
    let presentWeight = 0;
    for (const id of attendedIds) presentWeight += weightById.get(id) ?? 0;
      ...
      topics:
        phase === "pasado"
          ? sessionTopics.map((t) => ({
              ...
              // as plain attendees (their vote shown, still not tallied).
