...
beforeAll(async () => {
  ...
  // count as PRESENT in rosters/summaries, but their vote weight leaves tallies.
  ...
  await db.insert(votesTable).values([
    ...
    // Cast before checking out — visible in ballots but excluded from the tally.
...
describe("GET /history/sessions (admin)", () => {
  ...
  it("returns weighted results + attendance summary for each session", async () => {
    ...
    // Tally uses frozen weight_at_vote (favor frozen at 5, not current 2).
    expect(topic!.weights.favor).toBe(5);
    expect(topic!.weights.contra).toBe(3);
    expect(topic!.weights.abstención).toBe(1);
    expect(topic!.voteCount).toBe(3);
    // Closed topic, favor (5) > contra (3) -> approved.
...
describe("GET /public/history (unauthenticated)", () => {
  ...
  it("exposes per-member ballots for past topics; retired vote visible but excluded from tally", async () => {
    const res = await request(makeApp(historyRouter, fakeSession())).get("/api/public/history");
    const s = findSession(res.body) as PublicSession;
    const topic = s.topics.find((t) => t.topicId === topicId)!;

    // Tally unchanged: the retired member's favor (frozen 8) does NOT count.
    expect(topic.weights.favor).toBe(5);
    expect(topic.weights.contra).toBe(3);
