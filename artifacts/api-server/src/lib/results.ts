 ...
 * Retired members (checked out) are excluded entirely: pass their ids in `checkedOutIds`.
 */
export function computeTopicResult(
  ...
  // Retired members do not count toward results at all
  const countedVotes = votesForTopic.filter((v) => !checkedOutIds.has(v.userId));
  const voterIds = new Set(countedVotes.map((v) => v.userId));
  ...
  return { weights, percentages, approved, voteCount: countedVotes.length };
}
...
export function computeCandidateResult(
  ...
  const counted = detailVotes.filter((v) => !checkedOutIds.has(v.userId));
  ...
  const voters = new Set([...ballotUserIds].filter((id) => !checkedOutIds.has(id)));
  let sinVotoWeight = 0;
  let ausenteWeight = 0;
  let eligibleCount = 0;
  for (const member of members) {
