...
import { db, plenariasTable, attendanceTable } from "@workspace/db";
import { logger } from "./logger";

// Custom path so the connection routes through Replit's reverse proxy, which
...
export function initRealtime(
  ...
  io.use((socket, next) => {
    const session = (socket.request as { session?: { userId?: number } })
      .session;
    if (!session?.userId) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join", async (sessionId: unknown) => {
      ...
      try {
        const [plenaria] = await db
          .select({ id: plenariasTable.id })
          .from(plenariasTable)
        ...
        if (session.rol !== "admin") {
          const [attendance] = await db
            ...
            .where(
              and(
                eq(attendanceTable.sessionId, id),
                eq(attendanceTable.userId, session.userId),
